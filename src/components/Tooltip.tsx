...
const definitions: Record<string, { zh: string; en: string }> = {
  ...
  'open-subset-image': {
    zh: '把球面S²去掉两个极点后，得到的"去极球面"是S²的一个开子集。通过公式ψ把它映射（嵌入）到六维空间E⁶中，得到的曲面就是这个开子集的"像"。因为极点处公式中的tan趋于无穷，所以定义域必须排除极点。',
    en: 'Remove two poles from sphere S² to get a "punctured sphere"—an open subset of S². The formula ψ maps (embeds) it into 6D space E⁶; the resulting surface is the "image" of this open subset. The poles are excluded because tan in the formula diverges there.'
  },
};
